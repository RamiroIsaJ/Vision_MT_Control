...
class SegmentYeast:
    ...
    def well_main(self, des, img_r, ima_name, i, ctr_exp, bin_ref, prt_ref, cont_zero, x_, y_, radius_):
        ...
        print('Loading buffer of Percentage AREA...:  ' + str(i + 1) + ' of ' + str(self.buffer_size))
        k, mean_area = self.buffer_mean(i, prt_well)
        table = [['Percentage Area  : ', str(prt_well)],
                 ['Mean Area        : ', str(mean_area)]]
        for line in table:
            print('{:>10} {:>10}'.format(*line))
        print('')
        return prt_well, img_final, bin_ref, prt_ref, cont_zero, mean_area
